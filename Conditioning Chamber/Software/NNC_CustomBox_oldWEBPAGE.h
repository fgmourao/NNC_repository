...
<body>\n\
\n\
<center><h2>NNC_CustomBox V1.0</h2></center>\n\
<!-- \n\
<button type=\"button\" onclick=\"loadDoc()\">Request data</button></br>\n\
-->\n\
\n\
Digite sua senha: <input id=\"senha\" type=\"password\" value=\"\" />\n\
<button type=\"button\" onclick=\"enviarESP()\">Enviar Programacao</button>\n\
...
-->\n\
<div id=\"entrada\">\n\
 <p>ENTRADA DE DADOS SOBRE OS TRIALS</p></br>\n\
  Silencio entre trials ou silencio inicial (em segundos)<input id=\"Silence\" type=\"text\" value=\"\" /><br>\n\
  Sound onset (em segundos)<input id=\"SoundOnset\" type=\"text\" value=\"\" /> e \n\
  Sound duration (em segundos)<input id=\"SoundDuration\" type=\"text\" value=\"\" /><br>\n\
  Shock onset (em segundos)<input id=\"ShockOnset\" type=\"text\" value=\"\" /> e\n\
  Shock duration (em segundos)<input id=\"ShockDuration\" type=\"text\" value=\"\" /><br>\n\
  </br>\n\
  Carrier Frequency (Hz)<input id=\"CarrierFreq\" type=\"text\" value=\"\" /> e \n\
  Modulator Frequency (Hz)<input id=\"ModFreq\" type=\"text\" value=\"\" /><br>\n\
  ShockPulse High (ms)<input id=\"ShockPulseHigh\" type=\"text\" value=\"\" /> e \n\
  ShockPulse Low (ms)<input id=\"ShockPulseLow\" type=\"text\" value=\"\" /><br>\n\
  Sound Volume (%)<input id=\"SoundVolume\" type=\"text\" value=\"\" /><br>\n\
  </br>\n\
  <input id=\"SubmitTrial\" type=\"button\" onclick=\"adicionarTrial()\" value=\"Adicionar Trial\" /><br>\n\
</div>\n\
<hr>\n\
<textarea id=\"expTexto\" rows=\"4\" cols=\"50\"></textarea>\n\
<button type=\"button\" onclick=\"programaExperimento()\">CARREGAR</button></br>\n\
<hr><br>\n\
<div id=\"saidaHTML\">\n\
</div>\n\
\n\
...
for (var i=0;i<Trials[0].length;i++)\n\
  {\n\
  strTemp+=\"<hr>\";\n\
  strTemp+=\"<p>REGISTRO DO TRIAL - \" + i + \" tempo inicio (seg): \" + Trials[0][i] + \"s. \";\n\
  strTemp+=\"<br>Para apagar este registro clique ====> <button id=\\\"reg\" + i + \"\\\" type=\\\"button\\\" onclick=\\\"ClickDeleteReg(this)\\\">Delete</button></p>\";\n\
\n\
...
void ControleDueSkinner::Abort()
{
  // Disarm timers and put GPIOs in LOW state (except pinRSTarduino and pinSOUNDnot, which goes HIGH), indicating not to play sound and not to shock anymore.
    ...
    SPI.write16(0x001A,true); //Command: program parameters
    SPI.write16(0xA0A0,true); //Command: process data
  // Web page response.
...
void ControleDueSkinner::StartTrials()
{
  ...
  void *nada;
  iCountTrials=-1; // This variable is going to be incremented as soon as timer isr runs.
  if (silence[0]==0) isr_trial(nada); // Start
    else os_timer_arm(&(trial_timer), (uint32_t)(silence[0]*1000), 0);  // Arm timer
}
...
bool ControleDueSkinner::ProgramaVariaveis(int QuantosTrials, String strVar)
{
            ...
            // strVar contains: iNumberOfTrials;silence peroids; onset_sound instants, etc...
...
/*############################################################################################################
        Timmer Interrupt Routines
############################################################################################################*/
  
void isr_trial(void *pArg)
{
  ArduinoDueSkinner->iCountTrials++; // next trial
  ...
  ArduinoDueSkinner->ProgramaArduinoSPI(ArduinoDueSkinner->iCountTrials); // Program parameters of this trial
  delay(10); // Wait

  // Programming sound and shock of this trial
